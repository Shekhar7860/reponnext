...
class TeacherEditPost extends React.Component {
    ...
    createNewPost = () => {
        ...
        if (finalTimeSlot.length > 0 && currency) {
            this.setState({
                ...
                agent.Auth.updatePostById(info).then(res => {
                    console.log("updatePostById res", res);
                    this.props.router.back()
                }).catch(err => {
                    console.log("updatePostById err", err);
                    this.setState({
                        ...this.state,
                        loading: false
                    })
    ...
    render() {
        const { title, subject, teachStandard, houlyRate, description, languageMap, currency, duration, start_time, end_time,
        ...
        const { subjects, teaching_standards, languages, language } = this.props
        return <section>
                            ...
                            <div class="row gx-md-5">
                                ...
                                <div class="col-xl-3 col-lg-4 col-md-6 mt-4">
                                    ...
                                    <Multiselect
                                        ...
                                        selectedValues={language} // Preselected value to persist in dropdown
    ...
    componentDidMount() {
        const { user_info, postDetail, language, timeSlot } = this.props
        if (user_info) {
            let languageMap = new Map()

            let rowSundaySlot = new Map();
            let rowMondaySlot = new Map();
            let rowTuesdaySlot = new Map();
            let rowWednesdaySlot = new Map();
            let rowThursdaySlot = new Map();
            let rowFridaySlot = new Map();
            let rowSaturdaySlot = new Map();
            agent.setToken(user_info.token)

            language.forEach(element => {
                languageMap.set(element.id, element.id)
            });

            timeSlot.forEach(async element => {
                console.log("componentDidMount timeSlot element", element);
                switch (element.days) {
                    case 1: {
                        let time_slot = element.time_slot ? element.time_slot : []
                        time_slot.forEach(items => {
                    ...
                    case 3: {
                        ...
                        time_slot.forEach(items => {
                            ...
                            rowTuesdaySlot.set(mapKey, item)
                        });
                        break;
                    }
                    case 4: {
                        let time_slot = element.time_slot ? element.time_slot : []
                        time_slot.forEach(items => {
                            // console.log("time_slot.forEach", items);
                            let start_time = items.start_time;
                            let end_time = items.end_time;
                            const mapKey = `${start_time}${end_time}`
                            const item = {
                            ...
                            rowWednesdaySlot.set(mapKey, item)
                        });
                        break;
                    }
                    case 5: {
                        ...
                        time_slot.forEach(items => {
                            ...
                            rowThursdaySlot.set(mapKey, item)
                        });
                        break;
                    }
                    case 6: {
                    ...
                    case 7: {
                        ...
                        time_slot.forEach(items => {
                            ...
                            rowSaturdaySlot.set(mapKey, item)
                        });
                        break;
                    }
            ...
            this.setState({
                ...
                teachStandard: postDetail.teaching_standard,
                houlyRate: postDetail.hourly_rate,
                description: postDetail.description,
                currency: postDetail.currency,
                languageMap: languageMap,

                sundaySlot: rowSundaySlot,
                mondaySlot: rowMondaySlot,
                tuesdaySlot: rowTuesdaySlot,
                wednesdaySlot: rowWednesdaySlot,
                thursdaySlot: rowThursdaySlot,
                fridaySlot: rowFridaySlot,
                saturdaySlot: rowSaturdaySlot,

                sundaySlotchecked: rowSundaySlot.size > 0,
                mondaySlotchecked: rowMondaySlot.size > 0,
                tuesdaySlotchecked: rowTuesdaySlot.size > 0,
                wednesdaySlotchecked: rowWednesdaySlot.size > 0,
                thursdaySlotchecked: rowThursdaySlot.size > 0,
                fridaySlotchecked: rowFridaySlot.size > 0,
                saturdaySlotchecked: rowSaturdaySlot.size > 0,
            })
        }
    }
}
export default TeacherEditPost