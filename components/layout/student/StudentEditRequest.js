...
import { checkCreateEditSlot, manamusuDateFormatter, timeDateByUser, timestampToYyyyMmDd } from '../../../utils/date_contant';
import { NumberValidation } from '../../../utils/validation_contant';
import Multiselect from 'multiselect-react-dropdown';
import { hhmmToampm } from '../../../utils/work_contant';

class StudentEditRequest extends React.Component {
    ...
    setLanguageMap = (params) => {
        console.log("setLanguageMap", params, typeof params);
    ...
    updateRequest = () => {
        const { reqId, title, subject, currency, teachStandard, houlyRate, description, languageMap, selectedTimeSlot } = this.state
        let finalLanguage = []
        let finalTimeSlot = []
        ...
        if (title && finalTimeSlot.length > 0) {
            this.setState({
                ...this.state,
                loading: true
            }, () => {
                ...
                console.log("update request info", info);
                agent.Auth.updateRequestById(info).then(res => {
                    console.log("updateRequestById res", res);
    ...
    render() {
        const { reqId, title, subject, teachStandard, houlyRate, description, loading, currency, calendar_date, duration, start_time, end_time } = this.state
        const { languages, subjects } = this.props

        return <section>
            <div class="main-section">
                <div class="container">
                    <form onSubmit={(e) => { e.preventDefault(); this.updateRequest() }}>
                        <div>
                            <div class="main-heading me-4">
                                <h2 class="sub-heading mb-0 fs-24">Edit Request</h2>
                            </div>
                            ...
                            <div class="row gx-md-5">
                                ...
                                <div class="col-xl-3 col-lg-4 col-md-6 mt-4">
                                    ...
                                    <Multiselect
                                        ...
                                        selectedValues={this.props.language} // Preselected value to persist in dropdown
                                    ...
                                    />
                                    {/* <select class="form-select" aria-label="Default select example" onChange={(e) => this.setLanguageMap(e.target.value)}>
                                        <option value="">Add Language</option>
                                        {this.createDropdown(languages)}

                                    </select>
                                    <div class="time-slot mt-1">
                                        {this.getSelectedLanguage(languageMap, languages)}
                                    </div> */}
                                </div>
                        ...
                        <div class="mt-5">
                            ...
                            <div class="mt-5">
                                {loading ?
                                    ...
                                    : <button type="submit" class="btn btn-primary px-5" >UPDATE REQUEST</button>
                                }
                            </div>
                        </div>
                    </form>
    ...
    componentDidMount() {
        ...
        const { requestDetail, language, timeSlot, user_info } = this.props
        if (user_info) {
            agent.setToken(user_info.token)

            let langMap = new Map();
            let rowSelectedTimeSlot = new Map()

            language.forEach(element => {
                langMap.set(`${element.id}`, element.id)
            });

            timeSlot.forEach(element => {
                let timeDateBy = timeDateByUser(new Date(element.date))
                let mapKey = timeDateBy.getTime()
                let innerMap = new Map()

                element.time_slot.forEach(item => {
                    let innerMapKey = `${item.start_time}${item.end_time}`
                    innerMap.set(innerMapKey, { start_time: item.start_time, end_time: item.end_time })
                });
                rowSelectedTimeSlot.set(mapKey, innerMap)
            ...
            this.setState({
                ...this.state,
                reqId: requestDetail ? requestDetail.id : "",
                title: requestDetail ? requestDetail.title : "",
                subject: requestDetail ? requestDetail.subject : "",
                currency: requestDetail ? requestDetail.currency : "",
                teachStandard: requestDetail ? requestDetail.teaching_standard : "",
                houlyRate: requestDetail ? requestDetail.hourly_rate : "",
                description: requestDetail ? requestDetail.description : "",
                languageMap: langMap,
                selectedTimeSlot: rowSelectedTimeSlot,
            })
        }
