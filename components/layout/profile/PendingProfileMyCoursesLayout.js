...
class PendingProfileMyCoursesLayout extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    initialiseFilter = async (query) => {
        const items = await agent.Student.myCoursesPending(query)
    ...
    render() {
        ...
        return (populerFilterCount !== 0) ? <>
            <div class="main-heading mb-5">
                <h2 class="sub-heading mb-0 fs-24">My Courses / Pending</h2>
            </div>
            {this.getRowList()}
...
export default PendingProfileMyCoursesLayout