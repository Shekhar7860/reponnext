...
class MyPostProfileLayout extends React.Component {
    constructor(props) {
        super(props)
        console.log("MyPostProfileLayout props", props);
        this.state = {
    ...
    getUserRequest = () => {
        ...
        posts.forEach((element, index) => {
            rowDate.push(<TeacherPostLayout key={index} {...element} subjects={this.props.subjects} teaching_standards={this.props.teaching_standards} currency={this.props.currency} />)
        });
        return rowDate
    }
    prepearQuery = async () => {
    ...
    initialiseFilter = async (query) => {
        ...
        const items = await agent.Auth.getPost(`${query}&currency=${currency}&`)
            ...
            <div class="d-flex justify-content-between align-items-center flex-wrap">
                <div class="d-flex align-items-center mt-2 mt-sm-0">
                    <div class="main-heading me-4">
                        <h2 class="sub-heading mb-0 fs-24">My Posts</h2>
                    </div>
                </div>
...
export default MyPostProfileLayout