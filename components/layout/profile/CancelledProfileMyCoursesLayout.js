...
class CancelledProfileMyCoursesLayout extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    initialiseFilter = async (query) => {
        const items = await agent.Student.myCoursesDecline(query)
    ...
    render() {
        ...
        return (populerFilterCount !== 0) ? <>
            <div class="main-heading mb-5">
                <h2 class="sub-heading mb-0 fs-24">My Courses / Cancelled</h2>
            </div>
            {this.getRowList()}
...
export default CancelledProfileMyCoursesLayout